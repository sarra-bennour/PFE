...
const ImporterTracking: React.FC = () => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      // Debug: Afficher les valeurs de hsCode pour chaque demande
      if (demandes && demandes.length > 0) {
        demandes.forEach((demande: any, index: number) => {
          console.log(`Demande ${index + 1}:`, {
            reference: demande.reference,
            hsCode: demande.hsCode,
            productName: demande.productName
          });
        });
      }
      ...
      const formattedDeclarations: Declaration[] = demandes.map((demande: any) => {
        return {
          ...
          ngp: demande.hsCode || 'N/A',  // Ici hsCode est présent
